...
;!function (win, $, undefined) {
	var SVGMap = (function () {
		...
		SVGMap.prototype.options = {
			...
			mapWidth: 500,
			mapHeight: 400,
			stateColorList: ['F7C4A7', 'B8CCEA', 'F5EA9B', 'B2D8A1', 'F6B9B9', 'B2D8A1', 'B8CCEA'],
			...
			clickCallback: function (stateData, obj) { window.location.href="http://ic.xf9.com/map/show?name="+obj.name; },
			external: false
		};
		...
		SVGMap.prototype.render = function () {
			...
			for (var state in mapConfig.shapes) {
                ...
                var yy = obj.getBBox().y + (obj.getBBox().height / 2);
                ...
                r.text(xx, yy, obj.name).attr(textAttr);
